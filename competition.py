...
DEFAULT_ROTATION_IN_DEGREE = 72       #Rotation angle is in degree
...
def main():
    ...
    for i in range(0,5):
        ...
        # 7번 미션 패드를 찾았다면 정확하게 착륙
        #if mpad_id == 7:
            # 미션 패드와의 거리 측정
        #    distance_to_pad_x = tello.get_mission_pad_distance_x()
        #    distance_to_pad_y = tello.get_mission_pad_distance_y()
            # 보정값을 적용하여 더 정확하게 착륙
        #    while distance_to_pad_x > DISTANCE_CORRECTION or distance_to_pad_y > DISTANCE_CORRECTION:
        #        if distance_to_pad_x > DISTANCE_CORRECTION:
        #            tello.move_left(5) if distance_to_pad_x > 0 else tello.move_right(5)
        #            distance_to_pad_x -= 5
        #        if distance_to_pad_y > DISTANCE_CORRECTION:
        #            tello.move_forward(5) if distance_to_pad_y > 0 else tello.move_back(5)
        #            distance_to_pad_y -= 5

                # 높이를 5mm씩 낮추면서 미션 패드를 다시 인식하여 보정
        #        while True:
        #            tello.move_down(5)  # 높이를 5mm씩 낮춤
        #            time.sleep(1)
                    # 미션 패드 재인식
        #            new_mpad_id = tello.get_mission_pad_id()
        #            if new_mpad_id == 7:
